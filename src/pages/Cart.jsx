...
export default function Cart() {
  ...
  let {products , currency , cartItems , navigate ,  updateQuantity} = useContext(ShopContext);
      ...
      {cartData.map((item ,index)=> {
        ...
        return (
          <div className='py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4' key={index}>
            ...
            <img onClick={()=> updateQuantity(item._id,item.size,0)} src={assets.bin_icon} className='w-4 cursor-pointer sm:w-5' alt="" />
          </div>
        )
      })}
    </div>
